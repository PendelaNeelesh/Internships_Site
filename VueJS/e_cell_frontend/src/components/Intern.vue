<template>
  <div class="interncontainer">
    ...
    <div v-if="managerstatus" class="button">
      ...
      <button v-if="managerstatus" @click="modifyme(internship.auto_id)">
	Modify
      </button>
    </div>
  </div>
...
export default {
    ...
    computed:{
      ...
      url: function(){
        return `http://18.222.77.7:8000/users/apply?id=${this.internship.auto_id}&user=${this.getuser}`
      },
    },
    methods: {
        async deleteme(id){
